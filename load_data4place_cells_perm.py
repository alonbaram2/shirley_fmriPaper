...
if __name__ == '__main__':
    ...
    sample_data_file_name = 'place_sampled' + str(num_samples)
    if os.path.exists(sampled_auc_file_name + '.npy'):
        ...
        if os.path.exists(sample_data_file_name + '.npy'):
            print('sampled data file exists')
            sampled_p = np.load(sample_data_file_name + '.npy')
            Svd_AUC_obj = Svd_AUC(grid_cells=grid_cells, place_cells=place_cells, sampled_p_data=sampled_p)
        else:
        ...
        print(f"sampled place dells file shape: {sampled_p.shape}")
